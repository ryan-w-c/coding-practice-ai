...
function grid(rng: Rng, m: number, n: number, landPct: number): string[][] {
...
export function cases(rng: Rng) {
  ...
  out.push({ name: "all land", args: [grid(rng, 4, 4, 1)] });
  return out;
}

export function stress(rng: Rng) {
  return [{ name: "500x500", args: [grid(rng, 500, 500, 0.45)] }];
}

export function brute(g: string[][]): number {
